...
const Sidebar = ({ filteredMenu, opennav, open,logOut }) => {
    ...
    return (
        <aside className={`w-48 -translate-x-[138px] fixed transition transform ease-in-out duration-700 z-50 flex h-full bg-white ${open && ' w-48 -translate-x-[1px] '} `}>
            ...
            <div onClick={opennav} className={`-right-0 transition transform ease-in-out duration-500 flex border-4 border-white dark:border-[#0F172A] dark:hover:bg-blue-500 hover:bg-custom_blue-500 absolute top-2 p-1 rounded-full text-white hover:rotate-45 ${!open && 'text-black'}`}>
                        ...
                        return (
                            <Fragment key={hId}>
                                ...
                                {subHeaders.map(({ title, icon, submenu, submenuItems, line, id,navtext }, index) => {
                                    ...
                                    return (
                                        <Fragment key={index}>
                                            {!submenu && (<li key={id} className={`no-select flex flex-row items-center p-2 pl-6 space-x-3  hover:text-shark-900 hover:shadow-lg hover:bg-custom_blue-50 h-full w-full text-black transform ease-in-out duration-300  ${line && ' border-b mb-2'} ${active === navtext && ' bg-custom_blue-200 text-custom_blue-700 rounded'}`}
                                     ...
                                     `}
                                                ...
                                                >
                                                    <div className=" no-select flex flex-row items-center p-2 pl-6 gap-3 justify-start hover:text-shark-900 hover:shadow-lg hover:bg-custom_blue-50 h-full w-full" onClick={() => {
                                                        setOpenSubmenu(!openSubmenu);
                                                        setChoseNav(id)
                                                    }}>
                                                        <span className="w-4 h-4 ">{icon}</span>
