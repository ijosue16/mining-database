...
import { useUpdateBuyerMutation,useGetOneSupplierQuery } from "../states/apislice";
            ...
            <ActionsPagesContainer title={'Supplier Details'}
                ...
                {isLoading? <Spin/>:(    <div className="grid grid-cols-1 gap-3 w-full">
                        ...
                        <div className="w-full bg-slate-50 grid grid-cols-2 py-2 border-b items-center justify-between">
                            ...
                            <RiEditLine className=" text-2xl pb-1 justify-self-end" />
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            ...
                            {/* <div className=" col-span-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"> */}
                                  {suply.mineSites?.map(({name,code,_id,coordinates})=>(
                                    ...
                                    onclick={()=>{navigate(`/edit/minesite/${_id}`)}}/>
                            ...
                            <span className="flex items-center gap-2 bg-slate-50 w-fit p-1 rounded-md px-4 border" onClick={()=>{setShow(!show)}}>
                                    ...
                                    {show? <motion.form onSubmit={handleSiteSubmit} animate={show ? { opacity: 1, y: -10, display: "grid" } : { opacity: 0, y: 0, display: "none", }}  action="" className="grid grid-cols-1 sm:grid-cols-2 col-span-full gap-3 pb-9">
                                        ...
                                        <button type="submit" className="w-fit p-1 bg-slate-100 border rounded-md">ADD site</button>
                                    </motion.form>:null}
