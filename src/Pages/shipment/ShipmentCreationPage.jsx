...
import {decidePricingGrade} from "@/components/helperFunctions.js";
...
import { ScrollArea } from '@/components/ui/scroll-area';
import { Separator } from '@/components/ui/separator';
import { AlertCircle, Edit, Plus, Save, Trash } from 'lucide-react';
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { Alert, AlertDescription } from '@/components/ui/alert';
import ShipmentPreview from './ShipmentPreview';
import { useToast } from "@/hooks/use-toast.js";
...
const ShipmentManagementPage = () => {
    ...
    // Calculate shipment statistics for selected lots
                ...
                <div className="lg:col-span-2">
                    <Card className="mb-6">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Select lots to add to your shipment
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <ScrollArea className="h-96">
                                <Table>
                                    ...
                                    <TableBody>
                                        {availableLots.map(lot => (
                                            <TableRow key={lot._id}>
                                                <TableCell>{lot.lotNumber}</TableCell>
                                                <TableCell>{lot.entry?.mineralType || '-'}</TableCell>
                                                <TableCell>{lot.beneficiary || '-'}</TableCell>
                                                <TableCell>{lot.entry?.supplyDate ? formatDate(lot.entry.supplyDate) : '-'}</TableCell>
                                                <TableCell className="font-medium">{lot.remainingWeight.toFixed(2)}</TableCell>
                                                <TableCell>{lot[decidePricingGrade(lot.pricingGrade)] || '-'}</TableCell>
                                                <TableCell>
                                                    <Button
                                                        size="sm"
                                                        variant="outline"
                                        ...
                                        {selectedLots.map(lot => {
                                            ...
                                            return (
                                                <div key={lot._id} className="mb-4 p-3 border rounded-md">
                                                    <div className="flex justify-between items-center mb-2">
                                                        <div className="font-medium">
                                                            Lot #{lot.lotNumber} ({lot.entry?.mineralType || '-'})
                                                            {isExistingLot && (
                                                                <Badge variant="outline" className="ml-2">
                                                                    Existing
                                                                </Badge>
