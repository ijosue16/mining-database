...
import { RiEditLine, RiFileListFill, RiFileEditFill } from "react-icons/ri"
import { PiDotsThreeVerticalBold } from "react-icons/pi"
import { BiSolidEditAlt } from "react-icons/bi"
import { FaSave } from "react-icons/fa"
import { MdAddLocationAlt, MdDelete, MdOutlineClose, MdPayments } from "react-icons/md"
...
const ColtanEntryCompletePage = () => {
    ...
    const { data, isLoading, isError, isSuccess, error } = useGetOneColtanEntryQuery({ entryId });
    ...
    const EditableCell = ({ editing, dataIndex, title, record, children, ...restProps }) => {
        const input = <Input style={{ margin: 0 }} type={dataIndex === 'weightOut' || dataIndex === 'rmaFee' ? 'number' : 'text'} />;
        return (
            <td {...restProps}>
                {editing ? (
    ...
    return (
        <div>
            <ActionsPagesContainer title={'Coltan Details'}
                subTitle={'View Coltan detailes'}
                actionsContainer={
                    <AddComponent component={
                        <>

                            {isLoading ? <Spin /> : (<div className="flex flex-col gap-3 w-full">
                            ...
                            <Modal
                                ...
                                footer={[
                                    <span key="actions" className=" flex w-full justify-center gap-4 text-base text-white">
                                        <button key="back" className=" bg-green-400 p-2 rounded-lg" onClick={() => handleModelAdvance()}>
                                            Confirm
                                        </button>
                                        <button key="submit" className=" bg-red-400 p-2 rounded-lg" type="primary" onClick={() => setShowPayModel(!showPayModel)}>
                                            Cancel
                                        </button>
                                    </span>
                                ]}
