...
import dayjs from "dayjs";
import isBetween from "dayjs/plugin/isBetween"
import {Checkbox, message, Modal, Space, Table, DatePicker, Button, Badge, Tooltip} from "antd";
import {motion} from "framer-motion";
import {useNavigate, useParams} from "react-router-dom";
import ListContainer from "../../components/Listcomponents/ListContainer";
import {
...
const EntriesListPage = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "action",
            render: (_, record) => {
                const needsAttention = !record.output || record.output.length === 0;
                return (
                    <>
                        <div className="flex items-center gap-4">
                            {/*{!permissions.entry?.edit &&*/}
    ...
    // Count entries without lots for visual indicator in UI
    const entriesWithoutLots = dataz.filter(entry => !entry.output || entry.output.length === 0).length;
                    ...
                    <>
                        ...
                        <div className=" w-full overflow-x-auto h-full min-h-[320px]">
                            ...
                            <Table
                                ...
                                rowKey="_id"
                                rowClassName={(record) =>
                                    (!record.output || record.output.length === 0) ? 'bg-amber-50' : ''
                                }
                            />
                        </div>
                    </>
