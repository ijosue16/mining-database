...
const EditExistingFile = () => {
    ...
    const [convertToSFDT, {data, isSuccess, isLoading, isError, error}] = useConvertToSFDTMutation({url},
        { skip: url === null, refetchOnMountOrArgChange: true, refetchOnReconnect: true}
    );
    ...
    useEffect(() => {
        const convertFileToSfdt = async () => {
            const response = await convertToSFDT({body: {url}});
            if (response.data) {
